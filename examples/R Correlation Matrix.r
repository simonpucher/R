...
#2. params
# Pay attention currencys and stock do not start at the same day
TIMEFRAME = '2018-01-05'
WATCHLIST = c('FB', 'AMZN', 'NFLX', 'GOOG', 'GE', 'XOM')
getSymbols(WATCHLIST, from=TIMEFRAME)
...
# AMZN = getSymbols('AMZN', from=TIMEFRAME, auto.assign=FALSE)
...
stocks = as.data.frame(merge(FB$FB.Close, AMZN$AMZN.Close,NFLX$NFLX.Close, GOOG$GOOG.Close, GE$GE.Close, XOM$XOM.Close))
